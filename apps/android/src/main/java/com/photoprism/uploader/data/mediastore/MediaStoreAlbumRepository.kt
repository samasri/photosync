 ...
 */
class MediaStoreAlbumRepository(private val contentResolver: ContentResolver, private val synthetic: com.photoprism.uploader.lab.SyntheticLibrary? = null) {
    ...
    suspend fun loadAlbums(): List<Album> = withContext(Dispatchers.IO) {
        if (com.photoprism.uploader.BuildConfig.BUILD_TYPE == "experiment") {
            return@withContext requireNotNull(synthetic) { "Lab requires synthetic data" }.albums()
        }
